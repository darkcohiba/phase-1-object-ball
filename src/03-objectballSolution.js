...
//variables for teams and all players
...
function shoeSize(playerName){
    for (let x in allPlayers) {
        if (x === playerName) return allPlayers[playerName].shoe
    }
}
...
function playerNumbers(teamNam){
    let players = (homeTeam.teamName === teamNam ? homeTeam.players : awayTeam.players)
    let playerNumbers = [];
    for (let x in players){
        playerNumbers.push(players[x].number)
    }
...
//helper function
function bigShoe(){
    let bigPlayer = { shoe: null, player: null, rebounds: 0,}
    for (let x in allPlayers){
