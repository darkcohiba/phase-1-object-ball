...
function numPointsScored(name){
    for (let player in allPlayers) {
        if (player === name) return allPlayers[name].points
    }
}
...
function playersNumbers(teamTitle){
...
function bigShoe(){
    ...
    for (let x in allPlayers) { //this line will iterate over all players
        if (allPlayers[x].shoe > bigPlayer.shoe) { //if the player that is being looped over has a larger shoe size than the player currently holding the largest shoe size in
...
//prep for 12/19/21 object ball review
function gameObject(){
    return {
        home:{
            teamName: "Brooklyn Nets",
